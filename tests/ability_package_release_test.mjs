...
try {
  ...
  assert.equal(result.package, "kujo-ability@1.2.0");
  assert.equal(result.reproducible, true);
  assert.equal(result.signed, false);

  const names = (await readdir(output)).sort();
  assert.deepEqual(names, ["SHA256SUMS", "kujo-ability-1.2.0.provenance.json", "kujo-ability-1.2.0.spdx.json", "kujo-ability-1.2.0.tgz"]);
  ...
  const sbom = JSON.parse(await readFile(join(output, "kujo-ability-1.2.0.spdx.json"), "utf8"));
  ...
  const provenance = JSON.parse(await readFile(join(output, "kujo-ability-1.2.0.provenance.json"), "utf8"));
