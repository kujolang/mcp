...
The current machine-generated support tiers are in [the Ability host compatibility matrix](generated/ability-host-compatibility.md). Its source artifact records exact revisions, versions, commands, duration, output digests, and evidence links; generation fails closed when a required row is missing, failed, future-dated, stale, lacks an artifact, or does not cover the current MCP source. Available sibling Ability Gateway, Agents SDK, and Kujo Pi worktrees must also match their recorded revisions exactly.
...
| Kujo CMD local product | Release-candidate package and model run validated | Fresh network and offline-source setup, packed npm install, durable local runtime, 14-source catalog, profiles, skills, canonical CLI calls, approval, idempotency, concurrency, cancellation, receipts, failed verification, restart, and a real `ollama/glm-5.3:cloud` catalog call pass without a Command Code account session |
...
| Generic Streamable HTTP MCP | Protocol certified with a controlled fixture | The Ability Gateway Worker entrypoint passes initialize, discovery, read execution, authorization, isolation, approval, replay, conflict, concurrency, quota, schema, and body-boundary checks; production OAuth transport evidence remains the managed VS Code certificate |
| Agents SDK | Native conformant | Shared canonical Ability digest and receipt conformance passes in the committed SDK revision |
...
Generic Streamable HTTP is protocol-certified against the controlled Ability Gateway Worker fixture. The fixture injects authenticated principal properties and exercises initialization, discovery, read execution, authorization, isolation, approvals, idempotency, quotas, schemas, and bounded requests. It does not replace the separate production OAuth transport certificate or claim a particular third-party HTTP host UI.
