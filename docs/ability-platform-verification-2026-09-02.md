...
| `ability-gateway` | `9f223d4` | clean | Private Cloudflare Workers managed control plane and remote MCP transport |
...
| Ability Gateway | `npm run check`; `npm audit --audit-level=low`; `npx wrangler deploy --dry-run`; fresh local D1 migration | Passed 18 tests, types, boundaries, zero-vulnerability audit, deploy packaging, and 19-command schema migration. The original Codex Security snapshot reported 5 findings; the pushed checkout bounds provider bodies, revokes grants from signed Membership events, reserves idempotency atomically, meters approval/audit routes by subject and tenant, and deletes usage data after 30 days |
