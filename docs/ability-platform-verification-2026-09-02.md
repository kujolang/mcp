...
| `ability-gateway` | `8bfd1c0` | clean | Private Cloudflare Workers managed control plane and remote MCP transport with provisioned KV/D1 bindings |
...
| Ability Gateway | `npm run check`; `npm audit --audit-level=low`; `npx wrangler deploy --dry-run`; local and remote D1 migration | Passed 18 tests, types, boundaries, zero-vulnerability audit, deploy packaging, and the 19-command schema migration. Cloudflare KV and D1 resources were provisioned on the Free account and their binding IDs committed at `8bfd1c0`; the initial schema was applied remotely. The original Codex Security snapshot reported 5 findings; the pushed checkout bounds provider bodies, revokes grants from signed Membership events, reserves idempotency atomically, meters approval/audit routes by subject and tenant, and deletes usage data after 30 days |
