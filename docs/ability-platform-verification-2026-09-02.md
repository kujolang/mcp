...
- Concord's MCP version findings require an explicit product release decision: the framework project metadata is `1.0.0` while the unpublished portable Ability package and README badge are `1.1.0`. CMS's reported changelog `1.0.1` conflicts with the file's visible `1.1.0` release and appears to be a scanner interpretation issue; neither result was silently reclassified as a pass.
