...
const mcpResult = (value, isError = false) => ({ content: [{ type: "text", text: JSON.stringify(value) }], structuredContent: value, ...(isError ? { isError: true } : {}) });
...
createInterface({ input: process.stdin, crlfDelay: Infinity, terminal: false }).on("line", (line) => {
  if (!line.trim()) return;
  let request; try { request = JSON.parse(line); } catch { return fail(null, -32700, "parse error"); }
